...
impl MapVisualizer {
    ...
    fn render_resources_stats(f: &mut Frame, area: Rect, app: &App) {
        let (energy_count, mineral_count, _) = Self::calculate_resource_stats(app.map);
    ...
    fn render_robots_stats(f: &mut Frame, area: Rect, app: &App) {
        ...
        let robot_text = vec![
            ...
            Line::from(vec![Span::raw("")]),
            Line::from(vec![
                Span::styled("Explorateurs: ", Style::default().fg(Color::White)),
                Span::styled(format!("{}", explorers), Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
                Span::styled("Scientifiques: ", Style::default().fg(Color::White)),
        ...
        for robot in robots {
            if robot.x == x && robot.y == y {
                return match robot.robot_type {
                    RobotType::Explorer => ('E', Color::Red),
                    RobotType::Scientist => ('S', Color::Blue),
                };
            }
        }
